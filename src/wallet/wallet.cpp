...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CEndessCoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/endlesscoin/endlesscoin/pull/10340, but that change has been
    // postponed until after https://github.com/endlesscoin/endlesscoin/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-endlesscoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CEndessCoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CEndessCoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CEndessCoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CEndessCoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CEndessCoinAddress(address).ToString());
}
 ...
 * https://endlesscointalk.org/?topic=54527, or
 * https://github.com/endlesscoin/endlesscoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CEndessCoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CEndessCoinAddress(dest).ToString(), key);
}
