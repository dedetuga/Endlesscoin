...
The RPM spec file provided here is for EndessCoin-Core 0.12.0 and builds on CentOS
...
[EndessCoin git repository](https://github.com/endlesscoin/endlesscoin) but are not part of
...
Currently only `Source10` is used, for the example `endlesscoin.conf` file.
...
in the endlesscoin git repository. At present only an SVG version of the EndessCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the endlesscoin
...
needed when 0.12.1 is released, a proper fix is already in the EndessCoin git
master branch.
...
idea to build EndessCoin Core against a static version of that library compiled
...
The `endlesscoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `endlesscoin-server` package. At this point, most RPM based
...
The `endlesscoin-server` package should have SELinux support. How to properly do
...
EndessCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/endlesscoin/). He has been
packaging EndessCoin for Fedora at least since 2012.
...
4. I renamed the `endlesscoin` package that contains the Qt GUI to `endlesscoin-core` as
that appears to be how the general population refers to it, in contrast to
`endlesscoin-xt` or `endlesscoin-classic`. I wanted to make sure the general population
