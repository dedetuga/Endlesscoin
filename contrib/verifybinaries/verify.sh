...
###   endlesscoincore.org and endlesscoin.org and compares them.
...
RCSUBDIR="test"
HOST1="https://endlesscoincore.org"
...
if [ $? -ne 0 ]; then
   echo "endlesscoin.org failed to provide signature file, but endlesscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "endlesscoin.org and endlesscoincore.org signature files were not equal?"
