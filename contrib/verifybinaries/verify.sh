...
###   endlesschains.000webhostapp.com and endlesschains.000webhostapp.com and compares them.
...
WORKINGDIR="/tmp/endlesscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://endlesschains.000webhostapp.com"
HOST2="https://endlesschains.000webhostapp.com"
BASEDIR="/bin/"
VERSIONPREFIX="endlesscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'endlesscoin-',
...
if [ $? -ne 0 ]; then
   echo "endlesschains.000webhostapp.com failed to provide signature file, but endlesschains.000webhostapp.com did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "endlesschains.000webhostapp.com and endlesschains.000webhostapp.com signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the EndessCoin Core binary release signing key installed?"
   fi
