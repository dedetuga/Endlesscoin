#!/bin/bash
# Copyright (c) 2013 The EndessCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete EndessCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no EndessCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old EndessCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected EndessCoin 0.7 datadir."
    ;;
  3)
    echo "Detected EndessCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected EndessCoin 0.8 datadir."
    ;;
esac
